		...
		if(cmd.getName().equalsIgnoreCase("tp") && args.length == 1){
			if(player.hasPermission("notaro.tp") || player.hasPermission("notaro.*")){
				...
				try{
					...
					if(!TpBlockFile.contains(Target)){
						...
						target.sendMessage(ChatColor.DARK_AQUA + player.getDisplayName() + ChatColor.DARK_AQUA + " teleported to you" + "!");
					}else{
						player.sendMessage(ChatColor.RED + "You cannot tp to " + target.getDisplayName() + ChatColor.RED + " at this time.");
						target.sendMessage(ChatColor.DARK_AQUA + player.getDisplayName() + ChatColor.DARK_AQUA + (" tried to tp to you."));
					}
			...
			if(player.hasPermission("notaro.summon") || player.hasPermission("notaro.*")){
				...
				try{
					...
					target.sendMessage(ChatColor.DARK_AQUA + player.getDisplayName() + ChatColor.DARK_AQUA + " summoned you!");
				}catch(Exception error){
					player.sendMessage(ChatColor.RED + "That player is not online!");
				}
			}else{
